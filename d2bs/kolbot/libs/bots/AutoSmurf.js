	...
	//---------------------Process Give/Receive Lists---------------------
		...
		for (i in waypointsToShare) {
			...
			if (!portal) { // Don't share this Waypoint if someone else is already.
				...
				for (var j = 0 ; j <80 ; j += 1) { //Here is some wrong, it is too long time to wait. Dark-f < 720 ; j += 1) { // Wait up to 3 minutes Team Members to grab the Waypoint.
...
//QUESTS
	...
	this.radament = function () {
		...
		Town.move(NPC.Atma);				//Fix stuck at atma in act2
		atma = getUnit(1, NPC.Atma);		//Fix stuck at atma in act2
		atma.openMenu();
		me.cancel();
		//Pather.teleport = false;
		return true;
